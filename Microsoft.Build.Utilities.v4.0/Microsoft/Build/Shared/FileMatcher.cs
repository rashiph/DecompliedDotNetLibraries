namespace Microsoft.Build.Shared
{
    ...
    internal static class FileMatcher
    {
        ...
        private static readonly GetFileSystemEntries defaultGetFileSystemEntries = new GetFileSystemEntries(FileMatcher.GetAccessibleFileSystemEntries);
        ...
        {
            ErrorUtilities.VerifyThrow((filespec == null) || (regexFileMatch == null), "File-spec overrides the regular expression -- pass null for file-spec if you want to use the regular expression.");
            ErrorUtilities.VerifyThrow((filespec != null) || (regexFileMatch != null), "Need either a file-spec or a regular expression to match files.");
        ...
        internal static string GetLongPathName(string path, GetFileSystemEntries getFileSystemEntries)
        {
            ...
            ErrorUtilities.VerifyThrow(!HasWildcards(path), "GetLongPathName does not handle wildcards and was passed '{0}'.", path);
            ...
            for (int i = num; i < strArray.Length; i++)
            {
                ...
                else
                {
                    ...
                    ErrorUtilities.VerifyThrow(strArray3.Length == 1, "Unexpected number of entries ({3}) found when enumerating '{0}' under '{1}'. Original path was '{2}'", strArray[i], str2, path, strArray3.Length);
                    str2 = strArray3[0];
        ...
        private static string RegularExpressionFromFileSpec(string fixedDirectoryPart, string wildcardDirectoryPart, string filenamePart, out bool isLegalFileSpec)
        {
            ...
            do
            {
                ...
                ErrorUtilities.VerifyThrow(builder.Length <= length, "Expression reductions cannot increase the length of the expression.");
            }
