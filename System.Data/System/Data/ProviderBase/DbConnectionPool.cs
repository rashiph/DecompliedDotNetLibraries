    ...
    internal sealed class DbConnectionPool
    {
        ...
        private void CleanupCallback(object state)
        {
            ...
            while (this.Count > this.MinPoolSize)
            {
                ...
                {
                    DbConnectionInternal internal2 = this._stackOld.SynchronizedPop();
                    if (internal2 != null)
                    {
            ...
            if (this._waitHandles.PoolSemaphore.WaitOne(0, false))
            {
                while (true)
                {
                    DbConnectionInternal internal3 = this._stackNew.SynchronizedPop();
                    if (internal3 == null)
                    {
                        break;
                    }
        ...
        {
            DbConnectionInternal item = null;
            try
            {
        ...
        {
            DbConnectionInternal fromTransactedPool = null;
            int num;
            ...
            try
            {
                ...
                switch (num)
                {
                    ...
                    case 0:
                        ...
                        if ((fromTransactedPool == null) || fromTransactedPool.IsConnectionAlive(false))
                        {
                            goto Label_036C;
                        }
                        Bid.PoolerTrace("<prov.DbConnectionPool.GetConnection|RES|CPOOL> %d#, Connection %d#, found dead and removed.\n", this.ObjectID, fromTransactedPool.ObjectID);
                        this.DestroyObject(fromTransactedPool);
                        fromTransactedPool = null;
                        if (this._waitHandles.CreationSemaphore.WaitOne(this.CreationTimeout))
                        {
                            try
                            {
        ...
        private DbConnectionInternal GetFromTransactedPool(out Transaction transaction)
        {
            ...
            if ((null != transaction) && (this._transactedConnectionPool != null))
            {
                ...
                this.PerformanceCounters.NumberOfFreeConnections.Decrement();
                if (transactedObject.IsTransactionRoot)
                {
                    try
                    {
                        transactedObject.IsConnectionAlive(true);
                        return transactedObject;
                    }
                    catch
                    {
                        Bid.PoolerTrace("<prov.DbConnectionPool.GetFromTransactedPool|RES|CPOOL> %d#, Connection %d#, found dead and removed.\n", this.ObjectID, transactedObject.ObjectID);
                        this.DestroyObject(transactedObject);
                        throw;
                    }
                }
                if (!transactedObject.IsConnectionAlive(false))
                {
                    Bid.PoolerTrace("<prov.DbConnectionPool.GetFromTransactedPool|RES|CPOOL> %d#, Connection %d#, found dead and removed.\n", this.ObjectID, transactedObject.ObjectID);
                    this.DestroyObject(transactedObject);
        ...
        private void PoolCreateRequest(object state)
        {
            ...
            try
            {
                ...
                {
                    this.ReclaimEmancipatedObjects();
                    if ((!this.ErrorOccurred && this.NeedToReplenish) && (!this.UsingIntegrateSecurity || this._identity.Equals(DbConnectionPoolIdentity.GetCurrent())))
                    {
                        bool success = false;
                        int num = 3;
                        uint creationTimeout = (uint) this.CreationTimeout;
                        ...
                        try
                        {
                            ...
                            if (num == 0)
                            {
                                if (!this.ErrorOccurred)
                                {
                                    ...
                                    {
                                        DbConnectionInternal internal2 = this.CreateObject(null);
                                        if (internal2 == null)
                                        {
                                            return;
                                        }
        ...
        private void ProcessDeactivateQueue(object state)
        {
            ...
            try
            {
                ...
                foreach (DbConnectionInternal internal2 in objArray2)
                {
        ...
        private bool ReclaimEmancipatedObjects()
        {
            ...
            lock (this._objectList)
            {
                ...
                {
                    DbConnectionInternal internal2 = this._objectList[j];
                    if (internal2 != null)
                    {
            ...
            for (int i = 0; i < count; i++)
            {
                DbConnectionInternal internal3 = list[i];
        ...
        private class DbConnectionInternalListStack
        {
            ...
            internal int Count
            {
                get
                {
                    ...
                    lock (this)
                    {
                        for (DbConnectionInternal internal2 = this._stack; internal2 != null; internal2 = internal2.NextPooledObject)
                        {
                            num++;
                        }
                    }
        ...
        private sealed class TransactedConnectionPool
        {
            ...
            {
                DbConnectionPool.TransactedConnectionList list;
                DbConnectionInternal internal2 = null;
                bool flag = false;
            ...
            {
                DbConnectionPool.TransactedConnectionList list2;
                bool flag = false;
                lock (this._transactedCxns)
                {
                ...
                Transaction tx = null;
                DbConnectionPool.TransactedConnectionList list = null;
                try
                {
                    tx = transaction.Clone();
                    list = new DbConnectionPool.TransactedConnectionList(2, tx);
                    lock (this._transactedCxns)
                    {
                        if (flag = this._transactedCxns.TryGetValue(transaction, out list2))
                        {
