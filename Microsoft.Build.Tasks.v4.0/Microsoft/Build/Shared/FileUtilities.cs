    ...
    internal static class FileUtilities
    {
        ...
        internal static string GetTemporaryFile(string directory, string extension)
        {
            ...
            catch (Exception exception)
            {
                ...
                throw new IOException(Microsoft.Build.Shared.ResourceUtilities.FormatResourceString("Shared.FailedCreatingTempFile", new object[] { exception.Message }), exception);
            }
            return path;
        }
        ...
        internal static class ItemSpecModifiers
        {
            ...
            internal static string GetItemSpecModifier(string currentDirectory, string itemSpec, string modifier, ref Microsoft.Build.Collections.CopyOnWriteDictionary<string, string> cachedModifiers)
            {
                Microsoft.Build.Shared.ErrorUtilities.VerifyThrow(itemSpec != null, "Need item-spec to modify.");
                Microsoft.Build.Shared.ErrorUtilities.VerifyThrow(modifier != null, "Need modifier to apply to item-spec.");
                ...
                if (fullPath == null)
                {
                    ...
                    try
                    {
                        if (string.Compare(modifier, "FullPath", StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            ...
                            fullPath = Microsoft.Build.Shared.FileUtilities.GetFullPath(itemSpec, currentDirectory);
                        ...
                        else if (string.Compare(modifier, "RootDir", StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            ...
                            {
                                str2 = Microsoft.Build.Shared.FileUtilities.GetFullPath(itemSpec, currentDirectory);
                                ThrowForUrl(str2, itemSpec, currentDirectory);
                            }
                            fullPath = Path.GetPathRoot(str2);
                            if (!Microsoft.Build.Shared.FileUtilities.EndsWithSlash(fullPath))
                            {
                                Microsoft.Build.Shared.ErrorUtilities.VerifyThrow(Microsoft.Build.Shared.FileUtilitiesRegex.UNCPattern.IsMatch(fullPath), "Only UNC shares should be missing trailing slashes.");
                        ...
                        {
                            fullPath = Microsoft.Build.Shared.FileUtilities.GetDirectory(itemSpec);
                        }
                        else if (string.Compare(modifier, "Directory", StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            ...
                            {
                                str3 = Microsoft.Build.Shared.FileUtilities.GetFullPath(itemSpec, currentDirectory);
                                ThrowForUrl(str3, itemSpec, currentDirectory);
                            }
                            fullPath = Microsoft.Build.Shared.FileUtilities.GetDirectory(str3);
                            Match match = Microsoft.Build.Shared.FileUtilitiesRegex.DrivePattern.Match(fullPath);
                            if (!match.Success)
                            {
                                match = Microsoft.Build.Shared.FileUtilitiesRegex.UNCPattern.Match(fullPath);
                            }
                            if (match.Success)
                            {
                                Microsoft.Build.Shared.ErrorUtilities.VerifyThrow((fullPath.Length > match.Length) && Microsoft.Build.Shared.FileUtilities.IsSlash(fullPath[match.Length]), "Root directory must have a trailing slash.");
                        ...
                        {
                            flag = false;
                            FileInfo fileInfoNoThrow = Microsoft.Build.Shared.FileUtilities.GetFileInfoNoThrow(Microsoft.Build.Shared.EscapingUtilities.UnescapeAll(itemSpec));
                        ...
                        {
                            flag = false;
                            string path = Microsoft.Build.Shared.EscapingUtilities.UnescapeAll(itemSpec);
                            if (File.Exists(path))
                            {
                        ...
                        {
                            flag = false;
                            string str6 = Microsoft.Build.Shared.EscapingUtilities.UnescapeAll(itemSpec);
                            if (File.Exists(str6))
                            {
                        ...
                        {
                            Microsoft.Build.Shared.ErrorUtilities.VerifyThrow(false, "\"{0}\" is not a valid item-spec modifier.", modifier);
                        }
                    }
                    catch (Exception exception)
                    {
                        ...
                        Microsoft.Build.Shared.ErrorUtilities.VerifyThrowInvalidOperation(false, "Shared.InvalidFilespecForTransform", modifier, itemSpec, exception.Message);
                    }
                    Microsoft.Build.Shared.ErrorUtilities.VerifyThrow(fullPath != null, "The item-spec modifier \"{0}\" was not evaluated.", modifier);
                    if (flag)
                    {
                        if (cachedModifiers == null)
                        {
                            cachedModifiers = new Microsoft.Build.Collections.CopyOnWriteDictionary<string, string>(StringComparer.OrdinalIgnoreCase);
                        }
