        ...
        private static IList<PathSubsegment> ParseUrlSegment(string segment, out Exception exception)
        {
            ...
            while (startIndex < segment.Length)
            {
                ...
                if (num2 == -1)
                {
                    ...
                    if (str == null)
                    {
                        exception = new ArgumentException(string.Format(CultureInfo.CurrentUICulture, System.Web.SR.GetString("Route_MismatchedParameter"), new object[] { segment }), "routeUrl");
                        return null;
                    }
                    if (str.Length > 0)
                    {
